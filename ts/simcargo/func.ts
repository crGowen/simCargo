class SimCargoController {
    ...
    private static selectedPlane:CargoCraft;
    private static jUpdateRequired = false;
    ...
    static addToUiList(id:string, l:string, c:string, r:string, selectable: boolean, addition:string = "a") {
        // populate list
        ...
        if (addition=== "x" || addition=== "z") {
            let textHandle = short.create("div", "", ["scList__text", "scList__text--full"]);
    ...
    static saveProgress() {
        ...
        let bStr = "";
        for (let i = 0; i < SimCargoController.cargoCrafts.length; i++) {
            if (SimCargoController.cargoCrafts[i].getIsOwned()) bStr = bStr + "y";
            else bStr = bStr + "n";
        }
    ...
    static loadProgress() {
        ...
        let ownerships = localStorage.getItem("ownedPlanes");
        for (let i = 0; i < SimCargoController.cargoCrafts.length; i++) {
            if (ownerships[i]=== "y") SimCargoController.cargoCrafts[i].buy();
    ...
    static getPortByCode(code:string) {
        for (let i = 0; i < SimCargoController.cargoPorts.length; i++) {
            if (SimCargoController.cargoPorts[i].getCode() === code) return SimCargoController.cargoPorts[i];
        }
    }
            ...
            case "p":
                if (SimCargoController.cargoCrafts[index].getIsOwned() || SimCargoController.money >= SimCargoController.cargoCrafts[index].getPrice()) {
                    ...
                    if (SimCargoController.cargoCrafts[index]!== SimCargoController.selectedPlane){
                        ...
                        let tList = short.byClass("scList__item--selected");
                    ...
                    if (SimCargoController.cargoCrafts[index].getIsOwned()) short.byId("scRBtn").innerText = "Sell Selected Plane";
                    else short.byId("scRBtn").innerText = "Buy Selected Plane";
                }
                break;
        }
    ...
    static newGame(loc: string = "", startingCraft: number = 0) {
        ...
        if (loc.length === 4) SimCargoController.currentAirport = SimCargoController.getPortByCode(loc);
